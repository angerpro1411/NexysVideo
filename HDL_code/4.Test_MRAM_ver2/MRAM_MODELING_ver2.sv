...
#(
    ...
    */    
    ...
    parameter tEND_Z = 15    //Choosing END_Z is the time back to Z into DQ bus when one of signal go high.
...
(
    ...
    // // State machine for controlling reading recovery time.
    // typedef enum {IDLE,READING,BACK_2_HIGHZ  } state_t;
    // state_t PRE_ST,NX_ST;
    // logic i_CLK,;

    // always_ff @(posedge i_CLK)

//////////////// START::Testing with delay ///////////////////////////
    
    //Use to catch the reading moment
    logic READING_ON_UB = 1'b0;
    logic READING_ON_LB = 1'b0;
    always_latch begin
        // Base on timing order.

        if (!E_n && !G_n && !UB_n && W_n)  READING_ON_UB = 1'b1;
        else if (!UB_EN)                   READING_ON_UB = 1'b0;


        if (!E_n && !G_n && !LB_n && W_n)  READING_ON_LB = 1'b1;
        else if (!LB_EN)                   READING_ON_LB = 1'b0;
    ...
    // ----------------------------------------------------------
    always @(*) begin
        if (E_n && READING_ON_UB)        #(tEHQZ) UB_EN = 1'b0; // Chip disabled ? Hi-Z after tEHQZ
        else if (G_n && READING_ON_UB)   #(tGHQZ) UB_EN = 1'b0; // Output disabled ? Hi-Z after tGHQZ
        else if (UB_n && READING_ON_UB)  #(tBHQZ) UB_EN = 1'b0; // Upper byte disabled ? Hi-Z after tBHQZ
        else if (!W_n && READING_ON_UB)  #(tWLQZ) UB_EN = 1'b0; // Write mode ? bus turnaround, Hi-Z after tWLQZ
        else if (!E_n && !G_n && !UB_n && W_n)    UB_EN = 1'b1; // All conditions meet without delay
        else                                      UB_EN = 1'b0; // Define zero
    end
    ...
    // ----------------------------------------------------------
    always @(*) begin
        if (E_n && READING_ON_LB)        #(tEHQZ) LB_EN = 1'b0;
        else if (G_n && READING_ON_LB)   #(tGHQZ) LB_EN = 1'b0;
        else if (LB_n && READING_ON_LB)  #(tBHQZ) LB_EN = 1'b0;
        else if (!W_n && READING_ON_LB)  #(tWLQZ) LB_EN = 1'b0;
        else if (!E_n && !G_n && !LB_n && W_n)    LB_EN = 1'b1;
        else                                      LB_EN = 1'b0;
    end
...
//////////////// Testing without delay ///////////////////////////
    ...
    // // Check timing violation for reading recovery
    // time dq_back_Z_start;
    // logic lock_check_read_recovery = 1'b0;
    // logic release_lock = 1'b0;
    // always @(posedge E_n, posedge G_n, posedge UB_n, negedge W_n, release_lock)
    // begin
    //     if (READING_ON_UB && !lock) begin
    //         dq_back_Z_start = $time;
    //         lock_check_read_recovery = 1'b1;
    //     end
    //     if (release_lock) begin
    //         lock_check_read_recovery = 1'b0;
    //     end
    // end

    // always @(*) 
    // begin
    //     if ((!E_n && !G_n && !UB_n && W_n) && lock_check_read_recovery) begin
    //         if (($time - dq_back_Z_start) < tEHQZ) begin
    //             $warning("READING: *** Reading Recovery time VIOLATION - Waiting time = %0dns min = %0dns, Read old data @ %0t ns ***",
    //                                 ($time - dq_back_Z_start), tEHQZ, $time);
    //         end
    //     end 
    // end
